...
class TestTransformationOfSemanticData extends SeleniumTestCase_Base {
  ...
  public function testMyTestCase() {
    $this->login();
    $this->open($this->wgScriptPath . "/index.php?title=Helium&action=edit&mode=wikitext");
    ...
    for ($second = 0;; $second++) {
      if ($second >= 60)
        $this->fail("Element not present: //span[@class='fck_mw_property'][@property='has color::none'][text()='colorless']");
      try {
        if ($this->isElementPresent("//span[@class='fck_mw_property'][@property='has color::none'][text()='colorless']"))
          break;
      } catch (Exception $e) {
    ...
    for ($second = 0;; $second++) {
      if ($second >= 60)
        $this->fail("Text not present: " . $text);
      try {
